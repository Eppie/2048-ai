...
static inline board_t transpose(board_t x) {
	board_t t = (x ^ (x >> 12)) & 0x0000f0f00000f0f0ull;
	x ^= t ^ (t << 12);
...
static float score_move_node(eval_state &state, board_t board, float cprob) {
	...
	if(board != up) best = max(best, score_tilechoose_node(state, up, cprob));
	if(board != down) best = max(best, score_tilechoose_node(state, down, cprob));
	if(board != left) best = max(best, score_tilechoose_node(state, left, cprob));
	if(board != right) best = max(best, score_tilechoose_node(state, right, cprob));

	state.moves_evaled += 4;
	state.curdepth--;
...
struct board_state {
	...
	board_state( board_t b, int m, float f = 0.0f ) : board(b), move(m), res(f) {}

};

void* score_toplevel_move_thread( void* state ) {
	board_state* s = (board_state*)state;
	float res = score_toplevel_move( s->board, s->move );
	s->res = res;
}
...
int find_best_move(board_t board) {
	...
	for( move = 0; move < 4; move++ ) {
		states[move] = new board_state( board, move );
		pthread_create( &threads[move], NULL, score_toplevel_move_thread, (void *)states[move] );
	}

	for( move = 0; move < 4; move++ ) {
		pthread_join( threads[move], NULL );

		if( states[move]->res > best ) {
